...
async function loadTypes() {
  try {
    const res = await api.merchantRecycleBin.getTypes()
...
async function loadList() {
  loading.value = true
  try {
    const res = await api.merchantRecycleBin.getList(activeType.value, {
      page: page.value,
...
async function handleRestore(row: RecycleBinItem) {
  try {
    ...
    const res = await api.merchantRecycleBin.restore(row.type, row.id)
    if (res.code === 200) {
...
async function handlePhysicalDelete(row: RecycleBinItem) {
  try {
    ...
    const res = await api.merchantRecycleBin.physicalDelete(row.type, row.id)
    if (res.code === 200) {
...
async function handleBatchRestore() {
  ...
  try {
    ...
    const res = await api.merchantRecycleBin.batchRestore(activeType.value, ids)
    if (res.code === 200) {
...
async function handleBatchPhysicalDelete() {
  ...
  try {
    ...
    const ids = selectedRows.value.map((row) => row.id)
    const res = await api.merchantRecycleBin.batchPhysicalDelete(activeType.value, ids)
    if (res.code === 200) {
...
<template>
  <div class="merchant-recycle-bin">
    <AppHeader title="回收站" subtitle="查看并管理已删除的菜品、分类和优惠券">
